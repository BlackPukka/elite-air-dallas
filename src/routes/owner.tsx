...
function OwnerPage() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (pwd === "elite2024") { setOk(true); setErr(""); }
  ...
  if (!ok) {
    return (
      <main className="min-h-screen gradient-navy flex items-center justify-center p-6">
        <form onSubmit={submit} className="w-full max-w-md bg-card rounded-3xl p-8 shadow-luxe border border-border">
          ...
          <p className="text-muted-foreground text-sm mb-6">Private dashboard. Authorized personnel only.</p>
          <input
            type="password"
            value={pwd}
          ...
          <button className="w-full btn-elite is-primary h-12">
            Enter
          </button>
          <a href="/" className="block text-center text-xs text-muted-foreground mt-4 hover:text-foreground">← Back to site</a>
        </form>
      </main>
    );
  }
      ...
      <header className="bg-navy text-white">
        <div className="container mx-auto px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="size-9 rounded-lg gradient-gold flex items-center justify-center"><Wind className="size-5 text-navy" /></div>
            <div>
              <div className="font-display font-bold">Elite Air · Owner</div>
              <div className="text-[10px] uppercase tracking-[0.2em] text-gold">Private Dashboard</div>
            </div>
          </div>
          <div className="flex items-center gap-3 text-sm">
            <ShieldCheck className="size-4 text-gold" /> Authenticated
            <a href="/" className="ml-4 text-white/70 hover:text-gold">Exit</a>
          </div>
        </div>
      </header>
      <div className="container mx-auto px-6 py-10">
        <h1 className="font-display text-4xl font-bold mb-1">Good afternoon.</h1>
        <p className="text-muted-foreground mb-8">Here's how the business is performing right now.</p>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {kpis.map(k => (
            <div key={k.label} className="bg-card rounded-2xl border border-border p-6">
              <div className="flex items-center justify-between mb-3">
                <div className="size-10 rounded-xl gradient-gold flex items-center justify-center"><k.icon className="size-5 text-navy" /></div>
              ...
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground mt-1">{k.label}</div>
            </div>
          ))}
        </div>
        <div className="bg-card rounded-2xl border border-border overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {jobs.map((j, i) => (
              <div key={i} className="grid grid-cols-2 md:grid-cols-5 gap-4 p-5 text-sm items-center hover:bg-muted/40">
